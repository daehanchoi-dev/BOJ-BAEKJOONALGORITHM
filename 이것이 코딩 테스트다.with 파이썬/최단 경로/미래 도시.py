...
이때 소개팅의 상대방과 커피를 마시는 시간등은 고려하지 않는다고 가정한다.
예를 들어 N=5, X=4, K=5이고 회사 간 도로가 7개면서 각 도라가 다음과 같이 연결되어 있을 때를 가정할 수 있다.
(1번, 2번), (1번, 3번), (1번, 4번), (2번, 4번), (3번, 4번), (3번, 5번), (4번, 5번)
이때 방문 판매원 A가 최종적으로 4번 회사에 가는 경로를 (1번 - 3번 - 5번 - 4 번)으로 설정하면, 소개팅에도 참석할 수 있으면서
총 3만큼의 시간으로 이동할 수 있다. 따라것 이 경우 최소 이동 시간은 3이다.

입력조건 :
- 첫째 줄에 회사의 개수 N과 경로의 개수 M이 공배으로 구분되어 차례대로 주어진다. ( 1 <= N, M <= 100 )
